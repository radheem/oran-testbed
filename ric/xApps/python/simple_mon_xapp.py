...
class MyXapp(xAppBase):
    def __init__(self, config, http_server_port, rmr_port):
        ...
        # Publish KPM values to the metrics pipeline (Kafka by default; consumer
        # writes InfluxDB + MongoDB). Best-effort; never raises into callback.
    ...
    def my_subscription_callback(self, e2_agent_id, subscription_id, indication_hdr, indication_msg):
        ...
        # Report style 1 = cell-level aggregate (no per-UE id).
        if fields:
            self.metrics.write(fields, tags={"e2_node_id": e2_agent_id, "report_style": 1, "ue_id": "cell"})
